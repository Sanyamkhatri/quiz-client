...
const Question = () => {
  ...
  useEffect(() => {
    axios
      .get(`${process.env.REACT_APP_API_BASE_URL}/quiz/${quizId}`)
      .then((response) => {
  ...
  const handleNext = () => {
    if (quiz && quiz.questions && quiz.questions[0]) {
      ...
      if (currentQuestionIndex === pollQuestionsCount - 1) {
        ...
        axios
          .post(
            `${process.env.REACT_APP_API_BASE_URL}/quiz/${quizId}/submit`,
            { userAnswers }
          )
          .catch((error) =>
        ...
        axios
          .post(
            `${process.env.REACT_APP_API_BASE_URL}/quiz/${quizId}/impression`
          )
          .catch((error) =>
